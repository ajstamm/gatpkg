---
title: "GAT Tutorial 1: A walkthrough of GAT"
...
output: 
  rmarkdown::html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{GAT Tutorial 1 - A walkthrough of GAT}
...
*Note: For users who wish to modify GAT, a developer's manual is in progress. Developer options have been written into several of the package functions. Check the functions' help pages (linked from function names) and the [Technical Notes](../doc/gat-tech-notes.html) for more information. Users interested only in running GAT can ignore these links.*
...
```{r mergepic, fig.cap = 'Dialog: Identify Merge Type', out.width = '60%', echo = FALSE}
...
```{r ratepic, out.width = '60%', fig.cap = 'Dialog: Identify Rate Settings', echo = FALSE}
...
```{r kmlpic, out.width = '60%', fig.cap = 'Dialog: Save KML', echo = FALSE}
...
To follow the steps as shown, navigate to your documents folder. Enter "hftown_agg5k15k_popwt". 
...
If you choose to move on, click `Next >` to move to the next step, `< Back` to move to the previous step (selecting the rate settings), `Start over` to start from the beginning, or `Help` to get further guidance. To follow the steps as shown, click `Next >`.
