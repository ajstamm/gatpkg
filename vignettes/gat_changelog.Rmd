---
title: "Geographic Aggregation Tool (GAT) Change Log"
author: "Abigail Stamm"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Geographic Aggregation Tool (GAT) Change Log}
...
> 2015 and the first release of the R package for GAT.
...
* Rewrote and expanded quit code to not crash GAT.
* Standardized GAT-created variable names.
* Made GAT ID length dynamic and consistent.
* Expanded error messaging and made it more precise.
* Added dialogs to handle incorrectly entered data on the fly.


#### Resource changes

* Created over 20 functions with accompanying help files and examples.
* Developed several vignettes.
* Replaced statewide and Albany shapefiles with two smaller shapefiles, 
  hftown and hfblock.
* Removed simulated birth data and references to it.
* Added citation and license information.


#### User input changes

* Reordered and restructured user input dialogs.
* Revised user input dialogs and added function options to allow the 
  following:
    * maximum values for up to 2 aggregation variables
    * enforced boundaries for 1 boundary variable
    * enforced adjacent areas
    * population weighting using 2 methods
    * up to 3 exclusion criteria
    * KML file optional
    * saving and rerunning prior settings
    * comprehensive confirmation dialog that allows user to return to 
      previous steps
...
* Added all settings, aggregation warnings, and variables and files 
  created to the log. 
