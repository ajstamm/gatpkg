...
## GAT v2.1.0 (September 22, 2023)

* Rewrote `defineGATmerge()` to remove spdep entirely and rewrote merge fail messages for the log.
* Rewrote spdep sections to use sfdep or alternate code and removed dependency on spdep.
* Added check to filter out empty geographies.
* Removed `aggregateGATnb()`.

## GAT v2.0.1 (May 11, 2023)

* Added check for operating system in `locateGATshapefile()` and updated `runGATprogram()` accordingly.
* Added new advanced function options to make some functions compatible with the NYS Geomasking Tool (geomask package, still in development).
